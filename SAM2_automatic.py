...
mask_generator = SAM2AutomaticMaskGenerator(
    ...
    min_mask_region_area=200,  # ignore small masks
)
...
for m in masks:
    segmented_image, color_bgr = show_mask(m["segmentation"], segmented_image, random_color=True)
    objects_color.append(color_bgr)
...
for i, m in enumerate(masks):
    ...
    depths = depth_image[ys, xs] * depth_scale
    X = (xs - intr.ppx) * depths / intr.fx
    Y = (ys - intr.ppy) * depths / intr.fy
    Z = depths
    points_obj = np.vstack((X, Y, Z)).T
    ...
    for mu, cov in zip(mus, covs_scaled):
        ...
        object_ellipsoids.append((ellipsoid, mu, cov, objects_color[i]))
...
for i, (ellipsoid, mu, cov, color_bgr) in enumerate(object_ellipsoids):
    try:
        ...
        if np.linalg.norm(mu) < max_distance:
